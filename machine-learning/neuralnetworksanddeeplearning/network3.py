...
# Activition function for neurons
def linear(z):
    return z
def ReLU(z):
...
if GPU:
    print("Trying to run under a GPU. If this is not desired, then modify "
          "netword3.py\tto set the GPU to False.")
    try:
...
class Network(object):
    ...
    def __init__(self, layers, mini_batch_size):
        """Takes a list of `layers', describing the network architechure, and
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
            ...
            for minibatch_index in range(num_training_batches):
                ...
                if (iteration+1) % num_training_batches == 0:
                    validation_accuracy = np.mean(
                        [validation_mb_accuracy(i) for j in range(num_validation_batches)])
        ...
        print("Correspond test accuracy of {0:.2%}".format(test_accuracy))
...
class ConvPoolLayer(object):
    ...
    two, but for our purpose we'll always use them together, and it
    simplifies the code, so it make sense to combine them.
    """
    def __init__(self, filter_shape, image_shape, poolsize=(2,2), 
                 activition_fn=sigmoid):
        """`filter_shape` is a tuple of length 4, whose entries are the number
        of filters, the number of input feature maps, the filter height, and the
        filter width.

        `image_shape` is a tuple of lenght 4, whose entries are the
        ...
        self.activition_fn = activition_fn
    ...
    def set_inpt(self, inpt, inpt_dropout, mini_batch_size):
        ...
        pooled_out = downsample.max_poolsize_2d(
            input=conv_out, ds=self.poolsize, ignore_border=True)
        self.output = self.activition_fn(
...
class FullConnectedLayer(object):
    ...
    def set_inpt(self, inpt, inpt_dropout, mini_batch_size):
        ...
        self.inpt_dropout = dropout_layer(
            ...
            self.p_dropout)
        self.output_dropout = self.activition_fn(
...
def size(data):
    """Return the size of dataset 'data'."""
